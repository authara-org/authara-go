...
func TestConfigFromEnv_Defaults(t *testing.T) {
	...
	if cfg.Audience != "app" {
		t.Fatalf("expected default audience 'app', got %q", cfg.Audience)
	}
	if cfg.Issuer != "authara" {
		t.Fatalf("expected default issuer 'authara', got %q", cfg.Issuer)
	}
}
...
func TestConfigFromEnv_BaseURL(t *testing.T) {
	...
	if cfg.AutharaBaseURL != "http://example" {
		t.Fatalf("expected base url set")
	}
}

func TestWebhookHandlerFromEnv(t *testing.T) {
